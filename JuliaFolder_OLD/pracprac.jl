...
function solver(a,b,c,f,γ0,γL,L,P)
  Δx = L/P
  x = linspace(0,L,P+1)
...
U,x = solver(a,b,c,f,γ0,γL,L,P)
plot(x,U,"rs",xaxis,u(xaxis))
legend(("finite difference","exact solution");
        loc = "best")
grid(true)
...
for i=1:P+1
  @printf("%2.5lf     %2.10lf\n",x[i],abs(U[i]-u(x[i])))
  if max_err < abs(U[i]-u(x[i]))
    max_err = abs(U[i]-u(x[i]))
    max_err_x = x[i]
  end
end
println("The maximum error occurs at ",max_err_x," and is ",max_err)
